...
@section('content')
    <div class="page-content">
        ...
        <div class="analytics-container">
            <h2>Bienvenue sur votre nouvelle interface pour gerer le contenue de sensibilisation utilisateurs</h2>
            <h4>Ici vous pourez generer vos qr code de consigne intelligente pour le compte de vos clients, 
                gerer vos média et picto ainsi que le contenue de sensibilisation 
            </h4>
            <hr>
            <div class="row w-100 bg-success " style="height: 500px;">
                @php
                    $profils = App\Models\ProfilCustomer::with("consigns")->get();
                    // dd($profils);
                @endphp
                    @foreach ($profils as $profil)
                        ...
                        // dd($profil);
                        $i=0;
                            foreach ($profil->consigns as $consign) {
                            //    dd($consign);
                            
                            $ids.= ($i==0)?$consign->id  : ",".$consign->id;
                            $i++;
                            }
                        @endphp
                    <div class="col-3 col-md-6  col-lg-3 px-3">
                        <div class="card" style="display: flex; align-items:center; flex-direction:column;">
                            {!! QrCode::size(300)->generate('https://tismatek.com/?ids='.$ids) !!}
                            <h2>
                                {{$profil->name}}
                            </h2>
                        </div>
            ...
            <p><a href="#">Dès maintenant telecharger vos qr code selon les profil predefinis
            </a></p>
